...
export async function addStudent(req, res) {
    try {
        if (await Schools.exists({ _id: req.params.id }) == null) {
            ...
            await Schools.updateOne({ _id: req.params.id }, { $set: { 'classes': classes } }, (error) => { if (error) return res.status(500).send(error) }).clone().then(() => {
...
export async function removeStudent(req, res) {
    try {
        if (await Schools.exists({ _id: req.params.id }) == null) {
            ...
            classes.get(classID).get("students").splice(classes.get(classID).get("students").indexOf(studentID), 1);
            await Schools.updateOne({ _id: req.params.id }, { $set: { 'classes': classes } }, (error) => { if (error) return res.status(500).send(error) }).clone().then(() => {
...
export async function removeTeacher(req, res) {
    try {
        if (await Schools.exists({ _id: req.params.id }) == null) {
            ...
            classes.get(classID).get("teachers").splice(classes.get(classID).get("teachers").indexOf(teacherID), 1);
            await Schools.updateOne({ _id: req.params.id }, { $set: { 'classes': classes } }, (error) => { if (error) return res.status(500).send(error) }).clone().then(() => {
